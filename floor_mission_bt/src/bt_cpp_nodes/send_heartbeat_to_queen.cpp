...
RosServiceNode<ant_fleet_interfaces::srv::SendHeartbeatToQueen>(name, conf, params) {}
...
bool SendHeartbeatToQueen::setRequest(Request::SharedPtr& request)
{
  ...
  request -> robot_timestamp = _node->get_clock()->now().seconds();
...
{
  NodeStatus node_status == SUCCESS;
