...
SendDropoffPosition::SendDropoffPosition(const std::string &name,
                                         const NodeConfig &conf,
                                         const RosNodeParams &params)
    : RosServiceNode<ant_queen_interfaces::srv::DropoffPos>(name, conf,
                                                            params) {
...
SendDropoffPosition::getTransform(std::string drone_name) {
  ...
  try {
    ...
    if (auto node = node_.lock()) {
      RCLCPP_INFO(node->get_logger(),
                  "Transform from %s to %s:", source_frame.c_str(),
                  target_frame.c_str());
    ...
    transform_stamped.transform.rotation.w =
        2.0; // Set w to impossible value to flag error
  }
...
bool SendDropoffPosition::setRequest(Request::SharedPtr &request) {
  ...
  geometry_msgs::msg::TransformStamped map_to_attachment_point_tf =
      getTransform(drone_name);
  ...
  if (map_to_attachment_point_tf.transform.rotation.w <= 1.0) {
    ...
    if (auto node = node_.lock()) {
      RCLCPP_INFO(node->get_logger(), "[%s] Sending dropoff position..",
                  this->name().c_str());
    }
  } else {
    return false;
...
NodeStatus
SendDropoffPosition::onResponseReceived(const Response::SharedPtr &response) {
  if (response) {
