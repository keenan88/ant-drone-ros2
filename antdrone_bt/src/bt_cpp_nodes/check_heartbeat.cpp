...
CheckHeartbeat::CheckHeartbeat(const std::string &name, const NodeConfig &conf, const RosNodeParams &params) : RosServiceNode<antdrone_interfaces::srv::MissionHeartbeatSrv>(name, conf, params) {}

PortsList CheckHeartbeat::providedPorts() { return {OutputPort<std::string>("error_state")}; }
...
NodeStatus CheckHeartbeat::onResponseReceived(const Response::SharedPtr &response) {
  ...
  if (response->heartbeat_timeout_healthy) {
    ...
    if (auto node = node_.lock()) {
      RCLCPP_INFO(node->get_logger(), "[%s] Heartbeat timeout healthy ", this->name().c_str());
    }
  } else {
    if (auto node = node_.lock()) {
